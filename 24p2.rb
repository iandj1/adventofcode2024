...
def logic_str(gates, wire, swaps={}) # generate logic string for node recursively to exclude intermediate nodes
  ...
  str1 = logic_str(gates, in1, swaps)
  str2 = logic_str(gates, in2, swaps)
...
(0..44).each do |n|
  ...
  if logic_str(gates, gate, swaps) != logic_str(correct_gates, gate) # logic error found
    gates.keys.permutation(2).each do |gate1, gate2|
      new_swaps = known_swaps + [[gate1, gate2]]
      swaps = (new_swaps + new_swaps.map(&:reverse)).to_h
      begin
        if logic_str(gates, gate, swaps) == logic_str(correct_gates, gate) # error fixed
